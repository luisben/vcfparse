exports.getVersion30 = function (bufferString,callback) {
	...
	while ((beginVcard = bufferString.indexOf('BEGIN:VCARD', endVcard)) !== -1) {
		...
		for (var i=0 ; i<contactString.length ; ++i) {
			...
			if (contactString[i].substr(0,3) === 'ADR') {
				var firstSemi = contactString[i].indexOf(';');
				TYPE = contactString[i].substring(firstSemi,contactString[i].indexOf(';',firstSemi));
				ADR = contactString[i].substring(contactString[i].indexOf(';',firstSemi), contactString[i].length-1);
				ADR = escape(ADR);
